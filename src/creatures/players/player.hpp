...
public:
	...
	// V.I.P. functions
	void notifyStatusChange(std::shared_ptr<Player> player, VipStatus_t status) const;
	...
	*/
	...
	bool checkChainSystem() const {
		const bool toggleChain = g_configManager().getBoolean(TOGGLE_CHAIN_SYSTEM, __FUNCTION__);
		if (!toggleChain) {
			return false;
		}
