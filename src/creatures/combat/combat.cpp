...
void Combat::setupChain(const std::shared_ptr<Weapon> &weapon) {
	...
	switch (weaponType) {
		...
		case WEAPON_DISTANCE:
			setCommonValues(g_configManager().getFloat(COMBAT_CHAIN_SKILL_FORMULA_DISTANCE, __FUNCTION__), DIST_ATK_BOW, CONST_ANI_HOLY);
			break;
		case WEAPON_AMMO:
			setCommonValues(g_configManager().getFloat(COMBAT_CHAIN_SKILL_FORMULA_DISTANCE, __FUNCTION__), DIST_ATK_BOW, CONST_ANI_HOLY);
			break;
		case WEAPON_MISSILE:
			setCommonValues(g_configManager().getFloat(COMBAT_CHAIN_SKILL_FORMULA_MISSILE, __FUNCTION__), DIST_ATK_BOW, CONST_ANI_HOLY);
			break;
	}
	...
	if (weaponType == WEAPON_WAND) {
		...
		if (it != elementEffects.end()) {
			setPlayerCombatValues(COMBAT_FORMULA_LEVELMAGIC, 0, 0, g_configManager().getFloat(COMBAT_CHAIN_SKILL_FORMULA_WANDS_AND_RODS, __FUNCTION__), 0);
