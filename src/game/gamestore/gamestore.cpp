			...
			for (auto offerNode : categoryNode.children()) {
				...
				if (boost::iequals(type, "namechange")) {
					...
					if (!tmp->productId || !tmp->count) {
						printXMLError("Error parsing XML Item Offer - GameStore::loadFromXml", "data/XML/gamestore.xml", result);
						return false;
					} else {
						offer = tmp;
					}
					...
					if (!tmp->productId || !tmp->count) {
						printXMLError("Error parsing XML Stackable Item Offer - GameStore::loadFromXml", "data/XML/gamestore.xml", result);
						return false;
					} else {
						offer = tmp;
					}
					...
					tmp->count = (uint16_t)offerNode.attribute("count").as_uint();
					if (!tmp->productId || !tmp->count) {
						printXMLError("Error parsing XML Wrappable Item Offer - GameStore::loadFromXml", "data/XML/gamestore.xml", result);
						return false;
					} else {
						offer = tmp;
					}
					...
					if (!tmp->blessings.size()) {
						// no number was found
						printXMLError("Error Parsing XML bless offer - no blessnumber specified  - GameStore::loadFromXml", "data/XML/gamestore.xml", result);
						return false;
					}
					...
					if (tmp->days == 0) {
						printXMLError("Error parsing XML premiumtime offer type - required 'days' attribute not found - GameStore::loadFromXml", "data/XML/gamestore.xml", result);
						return false;
					}
...
HistoryStoreOfferList IOGameStore::getHistoryEntries(uint32_t account_id, uint32_t page) {
	...
	query << "SELECT * FROM `store_history` WHERE `account_id` = " << account_id << " ORDER BY `time` DESC LIMIT "
